...
nodoListaUsers* activarUsuarioPorId(char archivoUsuarios[], nodoListaUsers* ldl){
    int aActivar;
    nodoListaUsers* rta;
    int estado;
    char control = 's';
    while(control == 's'){
        printf("\nIngrese el id del usuario que desea activar: \n");
        scanf("%d", &aActivar);
        ...
        if(estado == 0){
            ...
            yellow();
            printf("\nEl id que ingreso es de un usuario que ya esta activo\n");
        ...
        while(seguidora){
            if(currentUser.id == seguidora->user.id){
                if(seguidora->playlist){
                    nodoListaPelis* seguidoraPelis = seguidora->playlist;
                    while(seguidoraPelis){
                        if(strcmp(seguidoraPelis->peli.titulo, pelicula) == 0){
...
void mostrarPerfil(usuario currentUser, nodoListaUsers* ldl){
    if(ldl){
        nodoListaUsers* seguidora = ldl;
        int flag = 0;
        ...
        if(seguidora){
            ...
            printf("\nMail...: %s\n", seguidora->user.mail);
            printf("\nTu Playlist:\n");
            mostrarListaPelis(seguidora->playlist);
        }
    }
}

void mostrarListaCurrentUser(usuario currentUser, nodoListaUsers* ldl){
    if(ldl){
        nodoListaUsers* seguidora = ldl;
        int flag = 0;
        while(flag == 0 && seguidora){
