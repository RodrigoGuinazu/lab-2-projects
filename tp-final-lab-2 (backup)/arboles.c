...
int resultadoDeBusqueda(nodoArbol* arbol, char aBuscar[]){
    int rta = 0;
    if(arbol){
        if(strstr(arbol->dato.titulo , aBuscar) != NULL && arbol->dato.estado == 1){
            rta = 1;
            mostrarPelicula(arbol->dato);
        }
        else{
            rta=resultadoDeBusqueda(arbol->izq, aBuscar);
            rta=resultadoDeBusqueda(arbol->der, aBuscar);
        }
    }
    return rta;
}
...
int validarTitulo(nodoArbol* arbol, char titulo[]){
    pelicula peliculaActual = buscarPorTitulo(arbol, titulo);
    return peliculaActual.id;
}

int validarGenero(char genero[]){
    if(strcmpi(genero, "ACCION") == 0 || strcmpi(genero, "COMEDIA") == 0 || strcmpi(genero, "DRAMA") == 0 || strcmpi(genero, "ROMANTICA") == 0 || strcmpi(genero, "SUSPENSO") == 0 || strcmp(genero, "TERROR") == 0){
        return 0;
    }else{
        yellow();
